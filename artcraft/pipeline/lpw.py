...
def preprocess_image(image, batch_size):
    w, h = image.size
    w, h = (x - x % 8 for x in (w, h))  # tasks to integer multiple of 8
...
def preprocess_mask(mask, batch_size, scale_factor=8):
    if not isinstance(mask, torch.FloatTensor):
        ...
        h, w = mask.shape[-2:]
        h, w = (x - x % 8 for x in (h, w))  # tasks to integer multiple of 8
...
class MyselfLPWStableDiffusionPipeline(StableDiffusionLongPromptWeightingPipeline):
    @torch.no_grad()
    def __call__(
            self,
            prompt: Union[str, List[str]],
            ...
            for i, t in enumerate(timesteps):
                ...
                if mask is not None:
                    ...
                    if add_predicted_noise:
                        ...
                        init_latents_proper = scheduler.add_noise(init_latents_orig, noise, torch.tensor([t]))
                    latents = (init_latents_proper * mask) + (latents * (1 - mask))
    ...
    def get_timesteps2(self, scheduler, num_inference_steps, strength, device, is_text2img):
        if is_text2img:
